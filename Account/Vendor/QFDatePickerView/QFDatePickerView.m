...
- (void)setViewInterface {
    ...
    for (int i = 0; i < 2; i ++) {
        ...
        [button setTitle:i == 0 ? @"cancel" : @"sure" forState:UIControlStateNormal];
...
- (void)setYearArray {
    ...
    for (NSInteger i = 1970; i <= currentYear ; i++) {
        NSString *yearStr = [NSString stringWithFormat:@"%ldYear",(long)i];
...
- (void)setMonthArray {
    ...
    if ([[selectedYear substringWithRange:NSMakeRange(0, 4)] isEqualToString:[NSString stringWithFormat:@"%ld",currentYear]]) {
        for (NSInteger i = 1 ; i <= currentMonth; i++) {
            NSString *monthStr = [NSString stringWithFormat:@"%02ldMonth",(long)i];
...
- (void)buttonTapped:(UIButton *)sender {
    if (sender.tag == 10) {
        ...
        if (onlySelectYear) {
            restr = [selectedYear stringByReplacingOccurrencesOfString:@"Year" withString:@""];
        } else {
            ...
            restr = [restr stringByReplacingOccurrencesOfString:@"Year" withString:@""];
            restr = [restr stringByReplacingOccurrencesOfString:@"Month" withString:@""];
        }
        backBlock(restr);
        [self dismiss];
    }
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (onlySelectYear) {//只选择年
        ...
        if (component == 0) {
            selectedYear = yearArray[row];
            if ([selectedYear isEqualToString:@"Now"]) {//至今的情况下,月份清空
